...
flags.DEFINE_integer("epochs", 53, "Number of epochs to train")
flags.DEFINE_float("learning_rate", 0.001, "Learning Rate for the model")
flags.DEFINE_string("model_savepath", "./models", "Path to save the trained model / savedmodel")
flags.DEFINE_string("checkpoint_dir", None, "Directory to save checkpoints. If None, uses <model_savepath>/checkpoints")
...
flags.DEFINE_float("exposure_loss_mean", 0.7, "Mean value for exposure loss")
...
def main(argvs):
    """Create/Validate model & checkpoint paths"""
    if not os.path.isdir(FLAGS.model_savepath):
        os.makedirs(FLAGS.model_savepath, exist_ok=True)

    checkpoint_dir = FLAGS.checkpoint_dir or os.path.join(FLAGS.model_savepath, "checkpoints")
    ...
    total_params = np.sum([np.prod(v.get_shape().as_list()) for v in dceNet.trainable_variables])
    print("Number of model parameters: {}".format(total_params))
    ...
    # Checkpointing setup
    # -------------------------
    # step will track number of completed epochs (so we can resume correctly)
    step = tf.Variable(0, dtype=tf.int64, name="epoch_step")
    ckpt = tf.train.Checkpoint(step=step, optimizer=optimizer, model=dceNet)
    ckpt_manager = tf.train.CheckpointManager(
        ckpt,
        directory=checkpoint_dir,
        max_to_keep=5  # change as you like
    )

    # Try to restore the latest checkpoint (auto-resume)
    if ckpt_manager.latest_checkpoint:
        print("Found checkpoint. Restoring from:", ckpt_manager.latest_checkpoint)
        ckpt.restore(ckpt_manager.latest_checkpoint).expect_partial()
        start_epoch = int(step.numpy())
        print(f"Resuming training from epoch {start_epoch + 1} (0-based counter = {start_epoch})")
        restored_from_checkpoint = True
    else:
        start_epoch = 0
        restored_from_checkpoint = False
        # If user wants to preload weights when starting fresh
        if FLAGS.preloadweights:
            print("Loading weights from path {}".format("/".join(x for x in FLAGS.preloadweights_path.split('/')[:-2])))
            # Attempt to load variables stored by tf.train.Saver-compatible format
            try:
                var_list = tf.train.list_variables(FLAGS.preloadweights_path)
                # skip the first if it's a meta attr depending on the checkpoint format
                for var, oldvar in zip(dceNet.trainable_variables, var_list):
                    loaded = tf.train.load_variable(FLAGS.preloadweights_path, name=oldvar[0])
                    var.assign(tf.convert_to_tensor(loaded, dtype=tf.float32))
                print("Preloaded weights successfully.")
            except Exception as e:
                print("Error preloading weights:", e)

    best_val_loss = None
    # If we restored, try to load a previously exported SavedModel's validation loss (optional)
    # We'll keep best_val_loss as None so the first improved validation will overwrite the savedmodel,
    # or you can store best_val_loss in a simple json in model_savepath if you prefer persistent best tracking.

    # -------------------------
    # Training loop (resumes automatically)
    # -------------------------
    for epoch in range(start_epoch, FLAGS.epochs):
        print("\nTraining on Epoch {}".format(epoch + 1))
        ...
        # Save checkpoint with optimizer state + epoch counter
        step.assign(epoch + 1)  # mark completed epochs
        saved_path = ckpt_manager.save()
        print(f"Checkpoint saved: {saved_path} (epoch {epoch + 1})")

        # Save exported SavedModel if validation improves
        current_val_loss = float(epoch_valloss_avg.result())
        if best_val_loss is None or current_val_loss < float(best_val_loss):
            print("New Best Validation Loss: {:.6f}".format(current_val_loss))
            print("Exporting new best SavedModel to", FLAGS.model_savepath)
            try:
                # Create a concrete function using a generic TensorSpec if your model __call__ supports it.
    ...
    print("Training finished. Final checkpoint saved to:", ckpt_manager.latest_checkpoint)
    print("To resume training, run the same command again; the script will auto-resume.")
